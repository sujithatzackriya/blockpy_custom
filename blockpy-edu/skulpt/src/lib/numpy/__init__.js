...
var $builtinmodule = function (name) {
            ...
            if (emits[idxLevel] < shape[idxLevel]) {
                ...
                if (idxLevel < uBound) {
                    ...
                    if (dtype === Sk.builtin.float_) {
                        str += Sk.ffi.remapToJs(new Sk.builtin.str(new Sk.builtin.float_(buffer[
                            i++])));
                    } else {
                        str += Sk.ffi.remapToJs(new Sk.builtin.str(buffer[i++]));
                    }
                    emits[idxLevel] += 1;
                }
            } else {
    ...
    */
    ...
    var sinh_f = function (x, out) {
        ...
        if (!np.math) {
            throw new Sk.builtin.OperationError("sinh requires mathjs");
        }
    ...
    var cosh_f = function (x, out) {
        ...
        if (!np.math) {
            throw new Sk.builtin.OperationError("cosh requires mathjs");
        }
    ...
    var tanh_f = function (x, out) {
        ...
        if (!np.math) {
            throw new Sk.builtin.OperationError("tanh requires mathjs");
        }
